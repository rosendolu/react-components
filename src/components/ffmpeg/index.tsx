...
export default function FFmpegComponent() {
  ...
  async function readFile() {
    // const fileName = localPath(`output.${form.getFieldValue('videoType')}`);
  ...
  async function scale(width) {
    const file = localPath(`output.${form.getFieldValue('videoType')}`);
    const command = `-v error -i ${file} -y -vf scale=${width}:-1 output_w_${width}.${form.getFieldValue('videoType')}`;
    await execCommands(command);
  }
    ...
    <div className="p-4">
      <div>
        <Space direction="horizontal" style={{ height: '100%' }}>
          ...
          {selectImg && <img src={selectImg} alt="" style={{ width: '300px', height: '300px' }} className="object-contain" />}
        </Space>
      </div>
